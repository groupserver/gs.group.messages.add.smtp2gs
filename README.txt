...
``siteUrl``
  **TODO:** Return the URL for the site
...
``x-original-to`` header. It involves `a hack`_ but it works.

A Hack
------

The code is built around a hack, where it adds the message as an element of
the ``REQUEST.form``. The request is needed for the can-post [#canpost]_ to
create the correct error-message for the email-notification that is sent if
the person who sent the message cannot post.
